...
void PROFILER::ProfileDataOutText(const char* szFileName)
{
	...
	fprintf(pFile, "---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
	fprintf(pFile, "| %-100s | %-15s | %-15s | %-15s | %-15s | %-15s | %-10s |\n", "Name", "TotalTime(100ns)", "Correction(100ns)", "Average (100ns)", "Min (100ns)", "Max (100ns)", "Call");
	fprintf(pFile, "---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
...
{
	// 프로파일링 결과 Reset중에는 측정하지 않게 하기 위해서
	if ((InterlockedIncrement(&PROFILER::state) & PROFILER::RELEASE_FLAG) != PROFILER::RELEASE_FLAG)
	{
		...
		{
			strcat_s(pSample->szName, PROFILE_SAMPLE::NAME_LEN, pFuncName);
			strcat_s(pSample->szName, PROFILE_SAMPLE::NAME_LEN - strlen(pFuncName), " ");
			strcat_s(pSample->szName, PROFILE_SAMPLE::NAME_LEN - strlen(pFuncName) - strlen(" "), pTagName);
			pSample->bUsed = true;
...
PROFILE_REQUEST::~PROFILE_REQUEST()
{
	...
	// Reset 혹은 Clear로 인해 프로파일링 시작에 실패햇다면 측정결과를 반영하지 않는다
